...
export function ShaderPlane({
  ...
  const scale = fillViewport
    ? ([viewport.width, viewport.height, 1] as [number, number, number])
    : ([1, 1, 1] as const);
  const planeArgs = fillViewport ? ([1, 1, 64, 64] as const) : ([2, 2, 32, 32] as const);
...
function EnergyRingViewportScaled() {
  const { viewport } = useThree();
  const r = Math.min(viewport.width, viewport.height) * 0.22;
  return <EnergyRing radius={r} position={[0, 0, 0.02]} />;
}

function ShaderScene() {
  return (
    <>
      <ShaderPlane fillViewport color1="#ff5722" color2="#0a0a0a" />
      <EnergyRingViewportScaled />
    </>
  );
}
...
export default PaperShaderBackdrop;
